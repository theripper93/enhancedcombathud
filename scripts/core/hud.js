...
export class CoreHUD extends Application {
  ...
  get actionBarWidth() {
    ...
    (this.element[0] ?? this._renderingElement)
  ...
  async bind(target) {
    ...
    if (
      supportedActorTypes.length &&
      !supportedActorTypes.includes(actorType)
    ) {
      ...
      if (!suppressWarnings)
        ui.notifications.error(
          localize("enhancedcombathud.err.unsupportedActorType").replace(
            "%t",
            actorType
          )
        );
  ...
  performModuleCheck() {
    ...
    const systemModuleElement = `<a href="https://foundryvtt.com/packages/enhancedcombathud-${
      game.system.id
  ...
  showTargetPickerGuide() {
    ...
    Hooks.once("renderDialog", (dialog, html) => {
      if (game.settings.get("enhancedcombathud", "dialogTheme"))
        html[0].classList.add("ech-highjack-window");
    });
  }
